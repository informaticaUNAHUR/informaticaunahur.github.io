...
const doc = new PDFDocument({
  ...
  info: {
    Title: "Manual de uso del CMS - Informática UNAHUR",
    Author: "Informática UNAHUR",
  },
});

doc.pipe(createWriteStream("manual-cms.pdf"));
...
function bullets(items) {
  items.forEach((item) => {
    ...
    doc
      ...
      .font("Helvetica-Bold")
      .fontSize(10)
      .text("•", ML + 8, y, { width: 14, lineBreak: false });
    doc
      .fillColor(GRAY)
      .font("Helvetica")
      .fontSize(10)
      .text(item, ML + 24, y, { width: PW - 24, lineGap: 2 });
    doc.y = doc.y + 2;
  });
  doc.moveDown(0.3);
}
...
function numbered(items) {
  items.forEach((item, i) => {
    ...
    doc
      ...
      .font("Helvetica-Bold")
      .fontSize(10)
      .text(`${i + 1}.`, ML + 8, y, { width: 18, lineBreak: false });
    doc
      .fillColor(GRAY)
      .font("Helvetica")
      .fontSize(10)
      .text(item, ML + 28, y, { width: PW - 28, lineGap: 2 });
    doc.y = doc.y + 2;
  });
  doc.moveDown(0.3);
}
...
function fieldRow(label, desc) {
  ...
  doc
    ...
    .font("Helvetica-Bold")
    .fontSize(9.5)
    .text(label + ":", ML + 8, y, { width: 150, lineBreak: false });
  doc
    .fillColor(GRAY)
    .font("Helvetica")
    .fontSize(9.5)
    .text(desc, ML + 164, y, { width: PW - 164, lineGap: 2 });
  doc.y = doc.y + 3;
}
...
doc
  .circle(doc.page.width - 50, 490, 130)
  .fillOpacity(0.05)
  .fill(TEAL);
doc.fillOpacity(1);
...
doc
  ...
  .font("Helvetica-Bold")
  .fontSize(36)
  .text("Manual de uso del CMS", 0, 200, {
    align: "center",
    width: doc.page.width,
  });
doc
  ...
  .font("Helvetica")
  .fontSize(14)
  .text("Guía para editar el contenido del sitio web", 0, 265, {
    align: "center",
    width: doc.page.width,
  });
doc
  ...
  .font("Helvetica")
  .fontSize(9)
  .text("pfahlerfranco.github.io/UnahurInformatica", 0, 308, {
    align: "center",
    width: doc.page.width,
  });
doc
  ...
  .font("Helvetica")
  .fontSize(8)
  .text("2026", 0, doc.page.height - 28, {
    align: "center",
    width: doc.page.width,
  });
...
para(
  "El CMS (Content Management System, o Sistema de Gestión de Contenidos) es una herramienta web que te permite editar el contenido del sitio de Informática UNAHUR sin necesidad de saber programar ni tocar código.",
);
para(
  "El sitio usa Decap CMS (antes conocido como Netlify CMS), que funciona directamente desde el navegador y guarda los cambios en GitHub. Cada vez que guardás, el sitio se actualiza automáticamente en pocos minutos.",
);
doc.moveDown(0.3);
...
para(
  "Para entrar al CMS necesitás una cuenta de GitHub con acceso al repositorio del proyecto.",
);
doc.moveDown(0.2);
...
warning(
  "Si no tenés acceso al repositorio GitHub, contactá al administrador del sitio para que te dé los permisos necesarios.",
);
...
para(
  "A continuación se describe cada colección: qué parte del sitio edita y qué campos tiene disponibles.",
);
...
doc.moveDown(0.3);
warning(
  "Modificar mal esta sección puede dejar el menú roto. No elimines items sin estar seguro/a.",
);

subHead("4.2  Banners");
para(
  'Gestiona el carrusel de banners de la página de inicio. Podés crear nuevos banners con el botón "Nuevo Banner".',
);
...
fieldRow(
  "Texto del botón",
  'Etiqueta del botón de acción (ej: "Ver carreras")',
);
...
fieldRow(
  "Estadísticas",
  "Lista de datos numéricos (valor, detalle, porcentaje de barra)",
);
fieldRow("Botón (opcional)", "Texto y URL del botón del bloque");

subHead("4.4  Sección Propuestas Formativas");
para(
  "Edita las pestañas de carreras del home (el selector con íconos y descripción).",
);
...
para(
  "Crea y edita las páginas individuales de cada carrera con su plan de estudios completo. Cada carrera tiene su propia URL.",
);
...
doc.moveDown(0.2);
para(
  "Cada materia dentro de un cuatrimestre tiene: Nombre, Slug (identificador), Área, Horas semanales, Horas totales y Correlativas.",
);

subHead("4.6  Sección Documentación y Recursos");
para(
  "Edita el bloque de documentación del home, con un post destacado grande y hasta 2 items laterales.",
);
fieldRow(
  "Post destacado",
  "Categoría, fecha, título, descripción, imagen, URL y autor",
);
fieldRow(
  "Items laterales",
  "Hasta 2 items con categoría, fecha, título, descripción, imagen y URL",
);
...
doc
  ...
  .font("Helvetica-Bold")
  .fontSize(9.5)
  .text("¿Cómo comunicarse?", ML + 8, doc.y);
doc.moveDown(0.15);
para(
  "Lista de medios de contacto. Cada item tiene: Nombre, Valor visible (ej: el email), URL, Ícono y si el link abre en pestaña nueva.",
);
doc
  .fillColor(DARK)
  .font("Helvetica-Bold")
  .fontSize(9.5)
  .text("Discord", ML + 8, doc.y);
doc.moveDown(0.15);
para(
  "Edita el bloque de Discord: Título, Subtítulo, URL de invitación e Ícono.",
);
doc
  .fillColor(DARK)
  .font("Helvetica-Bold")
  .fontSize(9.5)
  .text("Autoridades", ML + 8, doc.y);
doc.moveDown(0.15);
...
doc
  ...
  .font("Helvetica-Bold")
  .fontSize(9.5)
  .text("Trámites y consultas frecuentes", ML + 8, doc.y);
doc.moveDown(0.15);
para(
  "Lista de trámites con Nombre, Descripción y URL (link al formulario o sistema correspondiente).",
);

subHead("4.9  Página Materias");
para(
  'Crea y edita las materias del sitio. Se muestran en /materias/ con filtros por plan y carrera. Podés crear nuevas con "Nueva Materia".',
);
...
fieldRow(
  ...
  "Lista de preguntas y respuestas sobre la materia",
);

subHead("4.10  Página Novedades");
para(
  'Crea y edita las novedades del sitio. Las 3 más recientes aparecen también en el carrusel del home. Podés crear nuevas con "Nueva Novedad".',
);
fieldRow("Título", "Título de la novedad");
fieldRow("Fecha", "Fecha de publicación");
fieldRow(
  "Categoría",
  "Planes de estudio / Inscripciones / Eventos / Calendario / General",
);
fieldRow("Imagen", "Imagen de portada de la novedad");
fieldRow(
  "Descripción breve",
  "Resumen corto que aparece en la tarjeta del listado",
);
fieldRow(
  "Contenido",
  "Cuerpo completo de la novedad (editor de texto con formato)",
);

subHead("4.11  Página Horarios");
para(
  "Edita las fechas del calendario académico y los links de la planilla de horarios.",
);
fieldRow("Título", "Título que aparece en la página");
fieldRow(
  "1° Cuatrimestre",
  "Inicio de clases, Inscripción, Parciales, Finales, Fin de clases",
);
fieldRow("2° Cuatrimestre", "Las mismas fechas para el segundo cuatrimestre");
fieldRow(
  "Link embed Sheets",
  "URL del iframe (copiá desde Google Sheets: Archivo > Publicar en la web)",
);
fieldRow(
  "Link directo Sheets",
  "URL normal de la planilla para abrir en pestaña nueva",
);

// ── SECCIÓN 5 ─────────────────────────────────────────────────────────────────
sectionBanner(5, "Cómo subir imágenes");
para(
  "El CMS tiene un gestor de medios incorporado para subir y reutilizar imágenes en cualquier campo del sitio.",
);
doc.moveDown(0.2);
...
para(
  'Cuando terminás de editar, guardá los cambios desde el botón "Publish" (o "Guardar y publicar") en la parte superior del formulario.',
);
doc.moveDown(0.2);
...
doc.moveDown(0.4);
warning(
  "Si el cambio no aparece después de 5 minutos, verificá en github.com/Pfahlerfranco/UnahurInformatica que el commit haya llegado correctamente.",
);
doc.moveDown(0.3);
...
for (let i = 0; i < range.count; i++) {
  ...
  doc
    .moveTo(ML, doc.page.height - 38)
    .lineTo(ML + PW, doc.page.height - 38)
    .strokeColor("#dddddd")
    .lineWidth(0.5)
    .stroke();
  doc
    ...
    .fontSize(8)
    .text(
      "Manual de uso del CMS · Informática UNAHUR",
      ML,
      doc.page.height - 26,
      { width: PW - 30, lineBreak: false },
    );
  doc
    .fillColor("#aaaaaa")
    .font("Helvetica")
    .fontSize(8)
    .text(`Pág. ${i}`, ML + PW - 28, doc.page.height - 26, {
      width: 28,
      lineBreak: false,
    });
}
